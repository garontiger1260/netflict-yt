...
import * as a from 'react-icons/io'
// import ReactPlayer from 'react-player'
import dynamic from 'next/dynamic'
const ReactPlayer = dynamic( () => import("react-player/lazy"),{ssr:false})

const MovieDetail = ({ movie, showPlayer, setShowPlayer, trailerURL }) => {
    return (
        <>
            <Navbar />
            ...
            justify-center lg:pb-12'>
                    ...
                    <div className='flex space-x-3'>
                        <button className='bannerButton bg-white text-black' onClick={() => setShowPlayer(true)}>
                            <FaPlay className='h-4 w-4 text-black md:h-7 md:w-7' /> Play
                        </button>
                        <button className='bannerButton bg-[gray]/70'>
                            <a.IoMdInformationCircle className='h-5 w-5 text-black md:h-8 md:w-8' />
                            More Info
                        </button>
                    </div>
                </div>
